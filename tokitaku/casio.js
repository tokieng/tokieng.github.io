	...
	function() {
		...
		var enterROOT = function() {
			if (Disp.getDigits().getValue() < 0) {
				c.isError=true;
				Disp.E(true);
				var val = Disp.getDigits().getValue() * -1;
		...
		var enterOpePercentMul = function(key) {
			...
			[c.lastDigits, c.isError] = Common.calc(c.lastDigits, key, Disp.getDigits());
			c.inputDigits.reset();
			Disp.setDigits(c.lastDigits);
			Disp.E(c.isError);
			c.lastOpe = null;
		...
		var calcPercent_main = function(firstDigits, secondDigits) {
			...
			switch (c.lastOpe) {
				...
				case "+":
					var err1, err2, err3;
					// lastDigits / (1-centDigits) を計算
					[tmpDigits, err1] = Common.calc(1,"-",centDigits);
					[resultDigits, err2] = Common.calc(firstDigits, "/", tmpDigits);
					// その後 - を押下したときに、利益を表示するのであらかじめ計算しておく
					[c.percentPlusDigits, err3] = Common.calc(resultDigits, "-", firstDigits);
					nextStatus = "percent_+";
					c.isError = err1 | err2 | err3;
					break;

				case "-":
					// lastDigitsは、dispDigitsの何%アップか？ → (lastDigits - dispDigits)/dispDigits
					var err1, err2, err3;
					[resultDigits, err1] = Common.calc(firstDigits, "-", secondDigits);
					[resultDigits, err2] = Common.calc(resultDigits, "/", secondDigits);
					[resultDigits, err3] = Common.calc(resultDigits, "*", 100);
					c.isError = err1 | err2 | err3;
					break;
			}
			Disp.setDigits(resultDigits);
		...
		return {
			...
			"percent_+" : {
				// 基本的に、firstと同じ。- 押下時だけ異なる。
				numeric   : { next: null, func: addVal },
				comma     : { next: null, func: enterComma },
				"PM"      : { next: null, func: enterPlusMinus },
				ope       : { next: "first_ope", func: enterOpePercentPlus },
				"="       : { next: null, func: inputReset},
				"%"       : { next: null, func: inputReset},
				"ROOT"    : { next: null, func: enterROOT},
				memoryset : { next: null, func: memorySetOnly},
			...
			"percent_*" : {
				...
				comma     : { next: null, func: enterComma },
				"PM"      : { next: null, func: enterPlusMinus },
				ope       : { next: "first_ope", func: enterOpePercentMul },
				"="       : { next: "first", func: inputReset},
				"%"       : { next: null, func: inputReset},
				"ROOT"    : { next: null, func: enterROOT},
				memoryset : { next: null, func: memorySetOnly},
